...
# Applied to parse the entry urls, eg., hashtag pages.
	def hashtag_parse(self, response):
		...
		for item in self.item_parse(first_page, 'hashtag_parse'):
			yield item
		...
		for account in self.extract_accounts_inhash(first_page, response.request.url):
			next_user_page = self.base_user_url % urllib.quote_plus(account.encode('utf-8'))
	...
	def user_parse(self, response):
		first_page = response.body.replace('\n', '')
		for item in self.item_parse(first_page, 'user_parse'):
			yield item
...
# Parse the stream json
	def hashtag_stream_parse(self, response):
		json_stream = json.loads(response.body.encode('utf-8').replace('\n', ''))

		for item in self.item_parse(json_stream['items_html'], 'hashtag_stream_parse'):
			yield item

		for tag in self.extract_tags(json_stream['items_html'], response.request.url):
			next_tag_page = self.base_tag_url % urllib.quote_plus(tag.encode('utf-8'))
			yield scrapy.Request(next_tag_page, 
				callback = self.hashtag_parse)

		for account in self.extract_accounts_inhash(json_stream['items_html'], response.request.url):
			next_user_page = self.base_user_url % urllib.quote_plus(account.encode('utf-8'))
			yield scrapy.Request(next_user_page, 
				callback = self.user_parse)
	...
	def user_stream_parse(self, response):
		...
		for item in self.item_parse(json_stream['items_html'], 'user_stream_parse'):
			yield item
		...
		for account in self.extract_accounts_inuser(json_stream['items_html'], response.request.url):
			next_user_page = self.base_user_url % urllib.quote_plus(account.encode('utf-8'))
			yield scrapy.Request(next_user_page, 
				callback = self.user_parse)
...
# Extract accounts or tags in different pages
	...
	def item_parse(self, page_src, source):
		...
		for tweet_sel in Selector(text = page_src).xpath('//li[@class="js-stream-item stream-item stream-item"]/div'):
			try:
					item = TweetItem()
					item['user'] = tweet_sel.xpath('.//span[@class="username js-action-profile-name"]/b/text()').extract_first().encode('utf-8')
					item['content'] = ("".join(tweet_sel.xpath('.//div[@class="js-tweet-text-container"]/p//text()').extract())).encode('utf-8')
					item['quote_content'] = ("".join(tweet_sel.xpath('.//div[@class="QuoteTweet-text tweet-text u-dir"]//text()').extract())).encode('utf-8')
					item['timestamp'] = tweet_sel.xpath('.//a[@class="tweet-timestamp js-permalink js-nav js-tooltip"]/span[1]/@data-time-ms').extract_first()
					item['location'] = tweet_sel.xpath('./span[@class="Tweet-geo u-floatRight js-tooltip"]/a/span[2]/text()').extract_first(default = '').encode('utf-8')
					item['retweets'] = tweet_sel.xpath('.//div[@class="ProfileTweet-action ProfileTweet-action--retweet js-toggleState  js-toggleRt"]//span[@class="ProfileTweet-actionCountForPresentation"]/text()').extract_first(default = 0)
					...
					# print 'quote_content >>>',item['quote_content']
